function circle = makeSemiCircle(Nx, Ny, cx, cy, radius, arc_angle1, arc_angle2, plot_circle)
...
for point_index = 1:length(py)
    ...
    if (atan2(py(point_index) - cx, px(point_index) - cy) + pi) <= arc_angle2
...
while ( x < y - 1)
    ...
    for point_index = 1:length(py)
        ...
        if (arc_angle1<= angle_val && angle_val <= arc_angle2)
            circle(py(point_index), px(point_index)) = MAGNITUDE;
        end
    end
end
...
if arc_angle1 > 0
    circle(cx-radius,cy) = 0;
    circle(cx-radius+1,cy) = 0;
    circle(cx-radius-1,cy) = 0;
    circle(cx-radius,cy+1) = 0;
    circle(cx-radius,cy-1) = 0;
    circle(cx-radius+1,cy+1) = 0;
    circle(cx-radius+1,cy-1) = 0;
    circle(cx-radius-1,cy+1) = 0;
    circle(cx-radius-1,cy-1) = 0;           
